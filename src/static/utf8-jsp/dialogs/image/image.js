...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            var imageUrlPrefix= 'http://token-up-1257006810.cos.ap-beijing.myqcloud.com/';
            var imgUrl = '';
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                $.ajax({
                    ...
                    method:'post',
                    async:false,
                    url:'http://c.tokenup100.com/api/sys/cos/getSign',
                    // url:'http://58.87.125.242/api/sys/cos/getSign',
                    success:function(data){
                        file.filename = '/' + data.data.key;
                        actionUrl = actionUrl + '/' + data.data.key
                        data['key'] = '/' + data.data.key;
                        header['Authorization'] = data.data.authorization;
                        header['key'] = '/' + data.data.key;
                    }
                });
            });
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                var ue = UE.getEditor('ueditor',{
                    ...
                    enterTag:'br',
                    toolbars: [['fullscreen', 'source', '|', 'undo', 'redo', '|', 'bold', 'italic', 'underline', 'forecolor', 'backcolor', 'justifyleft', 'justifycenter', 'justifyright', 'justifyjustify','indent','|','searchreplace', 'removeformat','formatmatch','blockquote'],
                        ['paragraph', 'fontfamily', 'fontsize', 'rowspacingtop', 'rowspacingbottom','lineheight','insertorderedlist','insertunorderedlist','imagenone','imageleft','imageright','imagecenter','inserttable','insertimage', 'emotion','horizontal','spechars', 'link', 'unlink']]
                });
                var list = localStorage.image ? JSON.parse(localStorage.image) : [];
                for(var i = 0 ; i<list.length ; i++){
                    imgUrl = imageUrlPrefix + list[i];

                    ue.execCommand('insertHtml', '<img width="100%" src="'+ imgUrl + '"/>');
                }
